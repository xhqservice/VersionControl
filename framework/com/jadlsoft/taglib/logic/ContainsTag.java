	...
	private boolean condition() throws JspException {
		...
		if (scope == null) {
			...
			if (scopes.containsKey(scope.toLowerCase())) {
				bean = pageContext.getAttribute(name, ((Integer) scopes
						.get(scope.toLowerCase())).intValue());
			}
		}
		...
		if (bean instanceof String[]) {
			String[] modules = (String[]) bean;
			for (int i = 0; i < modules.length; i++) {
				if(modules[i].equals(value)){
					return true;
				}
			}
		}
